...
let leftMargin=200;
let topMargin=30;
let startTop=600+topMargin;

let startLeft=leftMargin;

function lineChart(rd) {
  ...
  var times_data=data.map(function(d) { return d[0] });
  var xScale = d3
  .scalePoint()
  .domain(times_data)
...
d3.select("svg")
    .append("g")
    .attr("class", "axis")
    .attr("transform", "translate(0,"+startTop+")")
    .call(xAxis)
    .append("text")
  ...
  let color = d3.scaleLinear().domain(indexArray).range(colorbrewer.Set2[6])
   if(data[0].length>2){
     for(let i =1; i<numCols;++i)
      drawLineAndPoint(data,color,i,xScale);
   } 
...
function drawLineAndPoint(data,color,i,xScale){
//   var sumstat = d3.nest() 
//   .key(d => d.media)
//   .entries(data);
...
var  yScale = d3.scaleLinear().domain([0, yMax]).range([startTop, topMargin])
ticks=10;
...
.append("g")
.attr("class", "axis"+i)
.attr("transform", "translate("+startLeft+",0)") //use variable in translate
.call(yAxis)
...
let maps=data.map(function(d) { return {x:d[0],y:d[i]} });
